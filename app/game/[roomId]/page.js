...
export default function GameRoom() {
  ...
  const [spyBasePoints, setSpyBasePoints] = useState(0);
  ...
  const handleRoomState = useCallback((data) => {
    ...
    setNumberOfSpies(data.numberOfSpies || 1);
    setTimeDuration(data.timeDuration || 300);
    
    if (data.spyIds) {
      setSpyIds(data.spyIds);
    }
  ...
  const handleRoundStarted = useCallback((data) => {
    ...
    setSpyBasePoints(data.spyBasePoints || 0);
    setNumberOfSpies(data.numberOfSpies || 1);
  ...
  useEffect(() => {
    ...
    socket.on('joined-success', (data) => {
      ...
      if (data?.numberOfSpies) setNumberOfSpies(data.numberOfSpies);
  ...
  const getScoreStructure = () => {
    return [
      { round: 1, normal: 2, spy: 6 },
      { round: 2, normal: 4, spy: 8 },
      { round: '3+', normal: 8, spy: 12 }
    ];
  };

  const getEarlyVotingBonusText = (totalPlayers) => {
    if (totalPlayers <= 3) {
      return [
        { position: '1st', bonus: 4 },
        { position: '2nd', bonus: 2 },
        { position: '3rd', bonus: 0 }
      ];
    }
    return [
      { position: '1st', bonus: 4 },
      { position: '2nd', bonus: 2 },
      { position: '3rd', bonus: 1 },
      { position: '4th+', bonus: 0 }
    ];
  };
  ...
  if (isAdmin) {
    ...
    const playerCount = normalPlayersOnly.length;
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 100%)', padding: '16px', color: 'white' }}>
        ...
        {/* HEADER */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', padding: '12px 16px', background: 'rgba(234, 179, 8, 0.1)', borderRadius: '10px', border: '1px solid #eab308', position: 'sticky', top: '15px', zIndex: '1000', background : '#ffffff', boxShadow: '0 2px 5px rgba(0, 0, 0, 0.1)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <span style={{ fontSize: '14px', color: '#1a1a2e' }}>Round</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ textAlign: 'right' }}>
              ...
              <div style={{ fontSize: '11px', color: '#1a1a2e' }}>👑 Admin</div>
            </div>
            <div style={{ 
            ...
            <div style={{ 
              ...
              {sortedPlayers[1] && (
                <div style={{ 
                  ...
                  <AvatarDisplay player={sortedPlayers[1]} size={50} border="2px solid #d1d5db" />
              ...
              {sortedPlayers[2] && (
                <div style={{ 
                  ...
                  <AvatarDisplay player={sortedPlayers[2]} size={50} border="2px solid #fb923c" />
                  <div style={{ fontSize: '48px', marginBottom: '8px', marginTop: '8px' }}>🥉</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
          ...
          <div style={{ background: '#2d2d44', padding: '20px', borderRadius: '12px', boxShadow: '0 4px 16px rgba(0,0,0,0.3)', maxHeight: '60vh', overflowY: 'auto' }}>
            <h2 style={{ margin: '0 0 16px', fontSize: '18px', color: '#eab308' }}>
              Players ({normalPlayersOnly.length}) {numberOfSpies > 1 && `• ${numberOfSpies} Spies`}
            </h2>
          ...
          <div style={{ background: '#2d2d44', padding: '20px', borderRadius: '12px', boxShadow: '0 4px 16px rgba(0,0,0,0.3)' }}>
            ...
            {gameStatus === 'lobby' && (
              <>
                ...
                <div style={{ 
                  ...
                  📊 {normalPlayersOnly.length} players = {maxRounds} rounds • {numberOfSpies} spy{numberOfSpies > 1 ? 's' : ''}
                </div>
                ...
                <div style={{ 
                  ...
                  ⏱️ Time: {formatTime(timeDuration)}
                </div>
              </>
            )}
            ...
            {gameStatus === 'playing' && (
              <div>
                <div style={{ textAlign: 'center', marginBottom: '16px' }}>
                  ...
                  {basePoints > 0 && (
                    <p style={{ color: '#eab308', fontSize: '12px', marginTop: '4px' }}>
                      💰 Normal: {basePoints} pts | Spy: {spyBasePoints} pts
                    </p>
                  )}
                </div>
                ...
                {/* Real-Time Voting - WITH SPY INDICATORS for Admin */}
                {Object.keys(voteCounts).length > 0 && playerCount > 3 && (
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ background: '#1a1a2e', padding: '12px', borderRadius: '8px', marginBottom: '12px' }}>
                  ...
                  <p style={{ margin: '6px 0', color: '#a855f7', fontSize: '13px' }}>
                    The Spy{result.spyIds?.length > 1 ? 's were' : ' was'}: <span style={{ fontWeight: 'bold' }}>{result.spyIds?.map(id => players.find(p => p.id === id)?.username).join(', ')}</span>
                  </p>
                  {result.votingRule && (
          ...
          {/* COLUMN 3: Game Info & Score Structure */}
          <div style={{ background: '#2d2d44', padding: '20px', borderRadius: '12px', boxShadow: '0 4px 16px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ marginBottom: '16px' }}>
              <h3 style={{ color: '#a855f7', marginBottom: '8px', fontSize: '14px' }}> Score Structure</h3>
              <div style={{ background: '#1a1a2e', padding: '12px', borderRadius: '8px', fontSize: '12px' }}>
                {getScoreStructure().map((round, idx) => (
                  <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', paddingBottom: '6px', borderBottom: idx < 2 ? '1px solid #4a4a6a' : 'none' }}>
                    <span style={{ color: '#9ca3af' }}>Round {round.round}:</span>
                    <div>
                      <span style={{ color: '#22c55e', marginRight: '8px' }}>N:{round.normal}</span>
                      <span style={{ color: '#ef4444' }}>S:{round.spy}</span>
                    </div>
                  </div>
                ))}
              </div>
              
              <h3 style={{ color: '#a855f7', marginBottom: '8px', fontSize: '14px', marginTop: '12px' }}>⚡ Early Voting Bonus</h3>
              <div style={{ background: '#1a1a2e', padding: '12px', borderRadius: '8px', fontSize: '12px' }}>
                {getEarlyVotingBonusText(playerCount).map((item, idx) => (
                  <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                    <span style={{ color: '#9ca3af' }}>{item.position} voter:</span>
                    <span style={{ color: item.bonus > 0 ? '#eab308' : '#6b7280' }}>+{item.bonus} pts</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, fontSize: '12px', color: '#9ca3af' }}>
                ...
                <li style={{ marginBottom: '6px' }}>2️⃣ {numberOfSpies} player{numberOfSpies > 1 ? 's are' : ' is'} the SPY</li>
        ...
        <div style={{ marginTop: '24px', marginBottom: '24px', textAlign: 'center', padding: '16px' }}>
            <button
        ...
        <div style={{ marginTop: '24px', textAlign: 'center', padding: '16px', borderTop: '1px solid #4a4a6a' }}>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 100%)', padding: '16px', color: 'white' }}>
      ...
      {/* HEADER */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', padding: '12px 16px', background: 'rgba(168, 85, 247, 0.1)', borderRadius: '10px', border: '1px solid #a855f7', position: 'sticky', top: '15px', zIndex: '1000', background : '#ffffff', boxShadow: '0 2px 5px rgba(0, 0, 0, 0.1)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <span style={{ fontSize: '14px', color: '#1a1a2e' }}>Round</span>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
                  ...
                  <div key={player.id} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', zIndex: 1 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ fontSize: '24px', opacity: isChampion ? 1 : 0.6 }}>
                          {index === 0 ? '' : index === 1 ? '🥈' : '🥉'}
                        </span>
                        <div>
                          <div style={{ fontSize: '18px', fontWeight: 'bold', color: isChampion ? 'white' : '#e5e7eb' }}>
                            {player.username}
                          </div>
                          {isChampion && (
                            <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.9)', fontWeight: 'bold', marginTop: '2px' }}>
                              🎉 CHAMPION 🎉
                            </div>
                          )}
                        </div>
                    ...
                    <div style={{ fontSize: '22px', fontWeight: 'bold', color: isChampion ? 'white' : '#fbbf24', zIndex: 1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '16px' }}>
          ...
          <div style={{ background: '#2d2d44', padding: '20px', borderRadius: '12px', boxShadow: '0 4px 16px rgba(0,0,0,0.3)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <h2 style={{ margin: 0, fontSize: '18px' }}>Players ({players.length}) {numberOfSpies > 1 && `• ${numberOfSpies} Spies`}</h2>
            </div>
          ...
          <div style={{ background: '#2d2d44', padding: '20px', borderRadius: '12px', boxShadow: '0 4px 16px rgba(0,0,0,0.3)', minHeight: '400px' }}>
                ...
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div style={{ 
                    ...
                    color: '#a855f7'
                     ...
                     {playerCount} players = {maxRounds} rounds • {numberOfSpies} spy{numberOfSpies > 1 ? 's' : ''} • {formatTime(timeDuration)}
                  </div>
                </div>
              </div>
            )}
            ...
            {gameStatus === 'playing' && (
              <div style={{ textAlign: 'center' }}>
                ...
                {/* Real-Time Voting Chart for Players */}
                {Object.keys(voteCounts).length > 0 && playerCount > 3 && myRole !== 'SPY' && (
                  <div style={{ background: '#1a1a2e', padding: '16px', borderRadius: '10px', marginTop: '20px', border: '1px solid #a855f7' }}>
                    <h3 style={{ margin: '0 0 12px', color: '#a855f7', textAlign: 'center', fontSize: '14px' }}> Real-Time Voting</h3>
                    <div style={{ display: 'grid', gap: '10px' }}>
                      {players.filter(p => p.id !== myId).map(player => {
                        const votes = voteCounts[player.id] || 0;
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ background: '#1a1a2e', padding: '20px', borderRadius: '10px', marginBottom: '20px', display: 'inline-block' }}>
                  ...
                  <p style={{ color: '#eab308', fontWeight: 'bold', fontSize: '16px', marginTop: '12px' }}>
                    The Spy{result.spyIds?.length > 1 ? 's were' : ' was'}: {result.spyIds?.map(id => players.find(p => p.id === id)?.username).join(', ')}
                  </p>
                </div>
        ...
        {/* Score Structure & Info - Always show */}
        <div style={{ marginTop: '16px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          <div style={{ background: '#2d2d44', padding: '16px', borderRadius: '12px', boxShadow: '0 4px 16px rgba(0,0,0,0.3)' }}>
            <h3 style={{ color: '#a855f7', marginBottom: '12px', fontSize: '16px' }}>📊 Score Structure</h3>
            <div style={{ display: 'grid', gap: '8px' }}>
              {getScoreStructure().map((round, idx) => (
                <div key={idx} style={{ 
                  ...
                  <span style={{ color: '#9ca3af', fontSize: '14px' }}>Round {round.round}</span>
                  <div style={{ display: 'flex', gap: '16px' }}>
                    <span style={{ color: '#22c55e', fontSize: '14px', fontWeight: 'bold' }}>Normal: {round.normal} pts</span>
                    <span style={{ color: '#ef4444', fontSize: '14px', fontWeight: 'bold' }}>Spy: {round.spy} pts</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ background: '#2d2d44', padding: '16px', borderRadius: '12px', boxShadow: '0 4px 16px rgba(0,0,0,0.3)' }}>
            <h3 style={{ color: '#a855f7', marginBottom: '12px', fontSize: '16px' }}>⚡ Early Voting Bonus</h3>
            <div style={{ display: 'grid', gap: '8px' }}>
              {getEarlyVotingBonusText(playerCount).map((item, idx) => (
                <div key={idx} style={{ 
                  display: 'flex', 
                  ...
                  <span style={{ color: '#9ca3af', fontSize: '14px' }}>{item.position} voter</span>
                  <span style={{ 
                    color: item.bonus > 0 ? '#eab308' : '#6b7280', 
                    fontSize: '14px', 
                    fontWeight: 'bold' 
                  }}>
                    +{item.bonus} pts
        ...
        {/* Back to Home Button - Below score structure boxes */}
        ...
        {/* Copyright */}
        <div style={{ textAlign: 'center', padding: '16px', borderTop: '1px solid #4a4a6a' }}>
          <p style={{ color: '#6b7280', fontSize: '12px' }}>
            © 2026 Alven Oblefias. All rights reserved.
          </p>
        </div>
